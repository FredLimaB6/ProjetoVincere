    ...
    <div id="game-queue">
        ...
        <?php if ($userQueueId !== null): ?>
            <div class="queue">
                ...
                <p><?php echo count($queues[$userQueueId]) . ' jogadores'; ?></p>
...
function game_system_process_queue() {
    ...
    if (!isset($_POST['action_type']) || !in_array($_POST['action_type'], ['join', 'leave'], true)) {
        error_log('[QueueSystem] Erro: Ação inválida recebida.');
    ...
    if ($action === 'join') {
        ...
        error_log("[QueueSystem] Jogador {$currentUserId} entrou na fila {$queueId}.");
    } elseif ($action === 'leave') {
        $message = $queueSystem->leaveQueue($currentUserId, $queueId);
        error_log("[QueueSystem] Jogador {$currentUserId} saiu da fila {$queueId}.");
    } else {
        error_log('[QueueSystem] Erro: Ação não reconhecida.');
    ...
    error_log("[QueueSystem] Estado atualizado: Jogador {$currentUserId} " . ($isInQueue ? 'ainda está' : 'não está mais') . " na fila {$queueId}.");
